...
- **ソフトウェアキャリブレーション**: 専用キーコードで ADC レンジを実測・EEPROM に保存。再コンパイル不要で JS16/JH16 の出力差を吸収
...
JS16 と JH16 は ADC 出力レンジが異なります（例: JS16 は 0〜1023、JH16 は Y 軸が 0〜784 等）。キャリブレーション機能を使うと、実際のスティック操作から min/max を自動測定し EEPROM に保存します。次回起動時に EEPROM から読み込むため、モデルが変わっても再コンパイル不要です。

### keymap.c への追加

カスタムキーコードを定義し、`process_record_user()` で処理します。
...
### Vial ファームウェアの場合: vial.json への追加

Vial GUI のキー割り当て画面にキャリブレーションキーを表示するには、`vial.json` に `customKeycodes` セクションを追加します。**順序は `enum custom_keycodes` の定義と必ず一致させてください。**

```json
{
  "customKeycodes": [
    {"name": "Calib Start", "title": "Joystick Calibration Start", "shortName": "CLB_S"},
    {"name": "Calib End",   "title": "Joystick Calibration End",   "shortName": "CLB_E"},
    {"name": "Calib Reset", "title": "Joystick Calibration Reset", "shortName": "CLB_R"}
  ],
  "lighting": "...",
  ...
}
...
Vial GUI の **Custom** タブに `CLB_S` / `CLB_E` / `CLB_R` が表示され、任意のキーに割り当てられます。

### キャリブレーション手順

1. **`JS_CALIB_START` キーを押す**
   - シリアルコンソールに `calibration started` が出力される
2. **スティックをゆっくり全方向に最大まで倒す（5〜10 秒）**
   - `JOYSTICK_DEBUG 1` 有効時は `CALIB: X=xxx~xxx Y=xxx~xxx` が約 1 秒ごとに流れる
3. **`JS_CALIB_END` キーを押す**
   - 実測された min/max が EEPROM に書き込まれる
   - コンソールに `calibration saved X=... Y=...` が出力される
4. **次回起動時から自動的にキャリブレーション値が使われる**

キャリブレーションを消去してデフォルト値に戻すには `JS_CALIB_RESET` を押します。

> **注意:** ステップ 2 ではスティックを中心に戻さず、全方向の端まで倒すことで正確な min/max が測定されます。範囲が小さすぎる場合（100 ADC 未満）はキャリブレーションが無効と判定されます。

### EEPROM アドレス

キャリブレーションデータは EEPROM アドレス `2048` 以降の 10 バイトに保存されます。他のデータと競合する場合は `config.h` で変更できます。

```c
#define JOYSTICK_CALIB_EEPROM_ADDR  2048  // デフォルト
```
...
5. EEPROM からレンジキャリブレーションを読み込む（保存済みの場合）
6. デバッグ有効時、キャリブレーション結果をコンソール出力
...
ADC レンジキャリブレーションを開始します。呼び出し後、スティックを全方向に最大まで倒してください。

---

### `void analog_stick_calibration_end(void)`

キャリブレーションを終了し、実測した min/max を EEPROM に保存します。測定範囲が 100 ADC 未満の場合は無効と判定し保存しません。

---

### `void analog_stick_calibration_reset(void)`

EEPROM のキャリブレーションデータを消去し、`JOYSTICK_ADC_X/Y_MIN/MAX` のデフォルト値に戻します。

---

### `bool analog_stick_is_calibrating(void)`

キャリブレーション中かどうかを返します。RGB LED の色変更など、視覚的なフィードバックを実装する際に使用できます。

```c
// 使用例: キャリブレーション中は赤に点滅
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (analog_stick_is_calibrating()) {
...
```c
// X/Y で出力レンジが違う場合の例（キャリブレーション未実施時のデフォルト値として機能する）
#define JOYSTICK_ADC_Y_MAX  784   // Y軸の最大値が小さい場合
```

**キャリブレーションとの関係:** `JOYSTICK_ADC_X/Y_MIN/MAX` はキャリブレーション未実施時のデフォルト値として使われます。キャリブレーション後は EEPROM の実測値が優先されます。`JS_CALIB_RESET` を押すとデフォルト値に戻ります。

| Parameter | Default | Description |
|---|---|---|
| `JOYSTICK_CALIB_EEPROM_ADDR` | `2048` | キャリブレーションデータの EEPROM 保存先アドレス（10 バイト使用） |
...
**ADC レンジキャリブレーション中:**
```
CALIB: X=0~1023 Y=0~784
```

| Field | Description |
|---|---|
| `X=min~max` | キャリブレーション中に記録された X 軸の実測レンジ |
| `Y=min~max` | キャリブレーション中に記録された Y 軸の実測レンジ |
...
- **Software calibration**: Dedicated keycodes measure the actual ADC range and save it to EEPROM — no recompile needed to support both JS16 and JH16
...
The JS16 and JH16 have different ADC output ranges (e.g. JS16: 0–1023, JH16 Y axis: 0–784). The calibration feature measures the actual min/max from real stick movement and saves it to EEPROM. On the next boot the saved values are loaded automatically, so no recompile is needed when switching models.

### keymap.c changes

Define custom keycodes and handle them in `process_record_user()`.
...
enum custom_keycodes {
    JS_CALIB_START = SAFE_RANGE,  // Start calibration
    JS_CALIB_END,                 // End calibration and save to EEPROM
    JS_CALIB_RESET,               // Erase EEPROM and revert to defaults
};
...
### Vial firmware: vial.json changes

To show the calibration keys in the Vial GUI key assignment screen, add a `customKeycodes` section to `vial.json`. **The order must match the `enum custom_keycodes` definition exactly.**
...
`CLB_S` / `CLB_E` / `CLB_R` will appear in the **Custom** tab of the Vial GUI and can be assigned to any key.

### Calibration procedure

1. **Press `JS_CALIB_START`**
   - The serial console prints `calibration started`
2. **Slowly tilt the stick to the maximum in all directions (5–10 seconds)**
   - With `JOYSTICK_DEBUG 1`, `CALIB: X=xxx~xxx Y=xxx~xxx` is printed every ~1 second
3. **Press `JS_CALIB_END`**
   - The measured min/max values are written to EEPROM
   - The console prints `calibration saved X=... Y=...`
4. **The calibrated values are loaded automatically on the next boot**

To erase calibration data and revert to defaults, press `JS_CALIB_RESET`.

> **Note:** During step 2, do not return the stick to center — tilt it all the way to the edge in every direction to capture accurate min/max values. If the measured range is less than 100 ADC units, calibration is rejected as invalid.

### EEPROM address

Calibration data is stored in 10 bytes starting at EEPROM address `2048`. If this conflicts with other data, override it in `config.h`.

```c
#define JOYSTICK_CALIB_EEPROM_ADDR  2048  // default
```
...
5. Loads range calibration from EEPROM (if previously saved)
6. If debug is enabled, prints calibration results to the console
...
Starts ADC range calibration. After calling this, tilt the stick to the maximum in all directions.

---

### `void analog_stick_calibration_end(void)`

Ends calibration and saves the measured min/max to EEPROM. If the measured range is less than 100 ADC units, the result is rejected and not saved.

---

### `void analog_stick_calibration_reset(void)`

Erases the EEPROM calibration data and reverts to the `JOYSTICK_ADC_X/Y_MIN/MAX` defaults.

---

### `bool analog_stick_is_calibrating(void)`

Returns `true` while calibration is in progress. Use this to implement visual feedback such as changing the RGB LED color.

```c
// Example: flash red during calibration
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    if (analog_stick_is_calibrating()) {
        rgb_matrix_set_color_all(255, 0, 0);
        return false;
...
```c
// Example: Y axis has a smaller maximum value (used as the default before calibration)
#define JOYSTICK_ADC_Y_MAX  784
```

**Relationship with calibration:** `JOYSTICK_ADC_X/Y_MIN/MAX` serve as the default values used before any calibration is performed. After calibration, the EEPROM values take precedence. Pressing `JS_CALIB_RESET` reverts to these defaults.

| Parameter | Default | Description |
|---|---|---|
| `JOYSTICK_CALIB_EEPROM_ADDR` | `2048` | EEPROM address where calibration data is stored (uses 10 bytes) |
...
| Field | Description |
|---|---|
| `X=min~max` | Measured X axis range recorded so far |
| `Y=min~max` | Measured Y axis range recorded so far |
